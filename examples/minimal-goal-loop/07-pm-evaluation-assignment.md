...
| --- | --- | --- |
| SOP-RECORDED | executable SOP is derived from accepted KR thresholds before candidate results are inspected | SOP section in evaluation artifact |
| FULL-RUN | all CLI, JSON, determinism, and text checks run | evaluator-owned raw evidence |
| KR-RESULTS | KR-1 and KR-2 receive binary results | per-KR evidence matrix |
