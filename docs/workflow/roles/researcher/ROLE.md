...
Produce the complete evidence base required to define the accepted Project OKR and full Product Contract. Do not research one isolated KR unless the complete assignment explicitly identifies it as a bounded annex to the global research artifact.

This role is configured as its own role instance. Do not switch roles inside this conversation.

## Result Contract / 结果契约

Consume the complete Project OKR, user decisions, current-project evidence, and public frontier sources when useful. Separate `repo_evidence`, `user_input`, `external_source`, `inference`, and `unknown`.

The one required primary professional artifact must cover:

- current product and repository facts relevant to every KR;
...
- a KR coverage matrix showing no accepted KR was silently omitted.
...
- Do not write Product Contract, architecture commitments, code, tests, evaluation verdicts, or Orchestrator state.
- Do not redefine KRs or split Engineering execution.
