...
The commitment is global: one complete Product Contract organized under every existing `KR-1...KR-N`. Product / PRD creates no second Objective or KR set. An artifact that resolves only one KR is incomplete.

产品承诺必须是全局的：在现有 `KR-1...KR-N` 下补全一份 Product Contract，不创建第二套 Objective 或 KR。只解决一个 KR 的产品文档不合格。
...
10. KR product coverage 是否完整且足够交给全局工程阶段 / whether every KR has complete product coverage ready for the global engineering stage
