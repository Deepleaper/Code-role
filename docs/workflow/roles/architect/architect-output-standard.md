...
The architecture artifact covers the complete Product Contract and must include KR-to-architecture traceability. It must not optimize one isolated KR while leaving the remaining product contract unmapped.

架构主产物必须覆盖完整 Product Contract，并包含 KR-to-architecture 追踪；不得只优化一个 KR 而遗漏其余产品契约。
