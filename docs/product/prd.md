...
The workflow must protect handed-off packets from silent drift without forcing strict handoff in daily use.
...
- Validator supports lightweight consumption from user-accepted `draft` packets.
- Validator can require strict `ready_for_next_role` plus packet lock when strict mode is requested.
- Validator can print a packet lock with file hashes for strict handoff.
...
- Should strict-handoff packet locks remain manually generated by validator command, or should a future strict mode generate them on demand?
